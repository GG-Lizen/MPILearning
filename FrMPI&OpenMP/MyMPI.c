...
void terminate(int id,              /* IN - Process rank */
...
void create_mixed_xfer_arrays(int id,      /* IN - Process rank */
...
void create_uniform_xfer_arrays(int id,      /* IN - Process rank */
                                int p,       /* IN - Number of processes */
...
void replicate_block_vector(void* ablock, /* IN - Block-distributed vector */
...
void read_checkerboard_matrix(char* s,            /* IN - File name */
            ...
            for (k = 0; k < grid_size[1]; k++) {
                ...
                if (grid_id == 0) {
                    ...
                    if (dest_id == 0) {
                        ...
                        MPI_Send(raddr, BLOCK_SIZE(k, grid_size[1], *n), dtype,
                                 dest_id, 0, grid_comm);
                    }
                    ...
                    MPI_Recv((*subs)[j], local_cols, dtype, 0, 0, grid_comm,
                             &status);
                }
            }
        }
...
void read_col_striped_matrix(char* s,            /* IN - File name */
...
void read_row_striped_matrix(char* s,        /* IN - File name */
                             ...
                             MPI_Comm comm)      /* IN - Communicator */
{
    ...
    *storage =
        (void*)my_malloc(id, local_rows * *n * datum_size);  //申请自己的内存
    *subs = (void**)my_malloc(id, local_rows * PTR_SIZE);
    ...
    if (id == (p - 1)) {
        for (i = 0; i < p - 1; i++) {
            x = fread(*storage, datum_size, BLOCK_SIZE(i, p, *m) * *n,
                      infileptr);
...
void read_block_vector(char* s,            /* IN - File name */
                       ...
                       MPI_Comm comm)      /* IN - Communicator */
{
    ...
    *v = my_malloc(id, local_els * datum_size);
    if (id == (p - 1)) {
        ...
        MPI_Recv(*v, BLOCK_SIZE(id, p, *n), dtype, p - 1, DATA_MSG, comm,
                 &status);
    }
}
...
void read_replicated_vector(char* s,            /* IN - File name */
                            void** v,           /* OUT - Vector */
...
void print_submatrix(void** a,           /* OUT - Doubly-subscripted array */
...
void print_subvector(void* a,            /* IN - Array pointer */
...
void print_checkerboard_matrix(void** a,           /* IN -2D matrix */
                               MPI_Datatype dtype, /* IN -Matrix element type */
...
void print_row_striped_matrix(void** a,           /* IN - 2D array */
                              MPI_Datatype dtype, /* IN - Matrix element type */
                              int m,              /* IN - Matrix rows */
                              int n,              /* IN - Matrix cols */
                              MPI_Comm comm)      /* IN - Communicator */
    ...
    if (!id) {
        ...
        if (p > 1) {
            ...
            for (i = 1; i < p; i++) {
                ...
                MPI_Recv(bstorage, BLOCK_SIZE(i, p, m) * n, dtype, i,
...
void print_block_vector(void* v,            /* IN - Address of vector */
...
void print_replicated_vector(void* v,            /* IN - Address of vector */
                             MPI_Datatype dtype, /* IN - Vector element type */
                             int n,              /* IN - Elements in vector */
                             MPI_Comm comm)      /* IN - Communicator */
{
